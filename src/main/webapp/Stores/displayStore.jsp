...
<div>
  <a href="storeservlet?service=insert">Insert Product</a>
</div>
    ...
    <table border="1">
      ...
      <%for (Stores st: storesVector){%>
        <tr>
          ...
          <td><%=st.getStreet()%></td>
          <td><%=st.getCity()%></td>
          <td><a href="storeservlet?service=update&id=<%=st.getStore_id()%>">update</a></td>
          <td><a href="storeservlet?service=delete&id=<%=st.getStore_id()%>">delete</a></td>
        </tr>
      <%}%>
    </table>
  <%}else{%>
